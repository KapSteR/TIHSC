...
set Library     "hls"
set IPName      "yuv_filter"
set Version     "1.0"
set DisplayName "Yuv_filter"
set Revision    "1505081318"
...
set Interfaces {
    ...
    ap_ctrl {
        type "ap_ctrl"
        ctrl_ports "ap_done ap_start ap_idle ap_ready"
        ctype {
            done {
                Type "bool"
                Width "1"
...
proc set_access { access obj } {
    # {{{
  # read-only, write-only, read-write, writeOnce, read-writeOnce 
  if        { $access eq "RW" } { set_property access "read-write" $obj
  } elseif { $access eq "R" } { set_property access "read-only" $obj
  } elseif { $access eq "W" } { set_property access "write-only" $obj
  } elseif { $access eq "WO" } { set_property access "writeOnce" $obj
  } elseif { $access eq "RWO" } { set_property access "read-writeOnce" $obj
...
if {[llength $SWDriverFiles] > 0} {
    ...
    foreach f $SWDriverFiles {
        ...
        set_property used_in {hw_handoff} $current_file
    }
}
...
## System Info
set user_parameters_list {clk_period 8.000000 machine 64 combinational 0 latency 80008 II x}
