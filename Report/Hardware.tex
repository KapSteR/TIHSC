...
\label{sec:hardware}
	...
	\label{sub:sensor}
		...
		\label{ssub:sensor_communication_protocol}
			\fxnote{ref til XV-11 hacking site}
	...
	\label{sub:motor_control}
		...
		\label{ssub:external_dual_h_bridge}
			...
			\begin{figure}[H]
				...
				\label{fig:DHB1}
			\end{figure}\fxnote{fix reference}
		...
		\label{ssub:pwm_controller}
			...
			The use of a 2 MHz PWM-clock is based on the choice of a 10 bit resolution and a recommendation from \cite{Digilent2014} \fxnote{fix reference}of a ~2 kHz PWM-cycle for the DHB1.
			...
			The procedure for controlling the motors from software is described in Section \fxnote{reference to motor control section under software}.A
